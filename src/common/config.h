...
struct ModelData {
  ...
  ModelData() : history(), tree(TREE_FOREST), ztype(Sampled), step_size(0.1), eta_t0(1),
  l2_parameter(0.0), time_series_parameter(1.0), label_sample_size(100),
  ...
  num_weight_types(1), num_label_types(1), approx_width(3),
  model_out("model"), dump_freq(10), init_to_I(false),
  calc_rae(true), calc_lbl(true), calc_bi(false), calc_through(false),
  cycles_so_far(0)
  {}
